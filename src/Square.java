...
public class Square {
	 ...
	 */
	protected boolean checkBounds(Square n){
		//for using this method below you have to switch the inpiut for checkBounds to Direction direction
		//and enter Direction.ROTATE for using in next method
		/*if (!ableToMove)
			return false;
		else if (square[0].getRow() == 0){
			return false;
		}
		// Each square must be able to move in that direction
		boolean answer = true;
		for (int i = 0; i < PIECE_COUNT; i++) {
			answer = answer && square[i].canMove(Direction.DOWN)&&square[i].canMove(Direction.UP)&&
					square[i].canMove(Direction.RIGHT)&&
					square[i].canMove(Direction.LEFT);
		}

		return answer;
	}*/
		if(n.getRow() == Grid.HEIGHT -1){
			System.out.println("Bottom bounds");
			return false;
		}
		if(n.getRow() == 0){
			System.out.println("Top bounds");
			return false;	
		}
		if(n.getCol() == Grid.WIDTH -1){
			System.out.println("Right bounds");
			return false;
		}
		if(n.getCol() == 0){
		...
		if (grid.isSet(n.getRow()+1, n.getCol()+1)||grid.isSet(n.getRow()-1, n.getCol()-1) ){
